...
// #cgo LDFLAGS: -L../longfellow-zk/reference/verifier-service/install/lib -lmdoc_static -lcrypto -lzstd -lstdc++
// #cgo CFLAGS: -I../longfellow-zk/reference/verifier-service/install/include
// #include <stdlib.h>
// #include <stddef.h>
// #include <stdint.h>
// #include <string.h>
...
// buildAttrs creates a single-attribute RequestedAttribute array.
func buildAttrs() *C.RequestedAttribute {
	attrs := C.make_attribute(1)
	cns := C.CString(namespace)
	cid := C.CString(elemID)
	cval := C.CBytes(elemValue)
	defer C.free(unsafe.Pointer(cns))
	defer C.free(unsafe.Pointer(cid))
	defer C.free(unsafe.Pointer(cval))
	C.set_attribute(attrs, 0, cns, cid, (*C.uint8_t)(cval), C.size_t(len(elemValue)))
	return attrs
}

// RunProver calls run_mdoc_prover. Returns (proofBytes, codeName, code).
func RunProver(circuit []byte, m *MintResult) ([]byte, string, int) {
...
func RunVerifier(circuit []byte, m *MintResult, proof []byte) (string, int) {
	cCirc := (*C.uint8_t)(C.CBytes(circuit))
	defer C.free(unsafe.Pointer(cCirc))
	cTr := (*C.uint8_t)(C.CBytes(m.Transcript))
	defer C.free(unsafe.Pointer(cTr))
