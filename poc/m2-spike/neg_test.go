...
// loadCircuit reads the same 1-attribute circuit main.go uses.
func loadCircuit(t *testing.T) []byte {
	t.Helper()
	c, err := os.ReadFile(circuitDir + circuitHash0)
	if err != nil {
		t.Fatalf("cannot read circuit: %v", err)
	}
	return c
}
...
//
// (2) is the trust boundary. If a proof verified under an issuer key that did
// not sign it, the one-bit verdict would be meaningless — this is exactly the
...
func TestAcceptThenReject(t *testing.T) {
	...
	proof, _, pCode := RunProver(circuit, m1)
	if pCode != 0 {
		t.Fatalf("prover on m1 failed: code %d", pCode)
	}

	// (1) correct issuer key -> accept.
	if _, vCode := RunVerifier(circuit, m1, proof); vCode != 0 {
		t.Fatalf("valid proof rejected under its own issuer key: code %d", vCode)
	}
	t.Log("valid proof accepted under correct issuer key")

	// (2) wrong issuer key (a second, independently minted credential's key,
	// so it is a well-formed P-256 point that simply did not sign this proof)
	// -> must NOT succeed.
	m2, err := Mint()
	if err != nil {
	...
	name, vCode := RunVerifier(circuit, &wrong, proof)
	if vCode == 0 {
...
func TestCannotForgeOver18FromUnder18(t *testing.T) {
	...
	orig := elemValue
	elemValue = []byte{0xF4} // CBOR false
	m, err := Mint()
	elemValue = orig
	if err != nil {
	...
	// Malicious holder asks the prover to open age_over_18 = true.
	elemValue = []byte{0xF5} // CBOR true — the lie
